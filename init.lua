...
-- Declare all the plugins your config requires
require("lazy").setup({
	-- Colorscheme
	{ "ellisonleao/gruvbox.nvim" },

	-- Treesitter (Required for your folding config)
	{
		...
		config = function()
			...
			if not configs_ok then
				vim.notify("nvim-treesitter.configs not available yet. Run :Lazy sync and restart.", vim.log.levels.WARN)
				return
			end
			configs.setup({
				ensure_installed = { "python", "typescript", "tsx", "javascript", "html", "css", "json", "lua", "bash", "vim" },
	...
	-- Autocompletion & Snippets (BULLETPROOF SETUP)
	{
		"saghen/blink.cmp",
		version = "*", -- Use stable release to avoid nightly Rust build issues on macOS
		...
		opts = {
			...
			-- CRITICAL: Force Lua engine to prevent macOS Rust binary crashes and ghost typing
			fuzzy = { implementation = "lua" },
		},
		config = function(_, opts)
			-- Safely require and setup to prevent "loop or previous error" crashes
			local ok, blink = pcall(require, "blink.cmp")
			if not ok then
				vim.notify("blink.cmp failed to load. Run ':Lazy build blink.cmp' or check logs.", vim.log.levels.ERROR)
				return
			end
			blink.setup(opts)
			
			-- Safely set LSP capabilities for Neovim 0.10+
			local cap_ok, capabilities = pcall(blink.get_lsp_capabilities)
			if cap_ok then
...
-- Transparency setup wrapped in ColorScheme autocmd to persist on reload/colors
vim.api.nvim_create_autocmd("ColorScheme", {
	...
	callback = function()
		local groups = {
			...
			"NvimTreeSignColumn", "NvimTreeEndOfBuffer", "FloatingTermNormal", "FloatingTermBorder"
		}
...
-- Trigger once for the initial colorscheme load
...
local function file_type()
	...
	local icons = {
		...
		scss = "\u{e749} ", json = "\u{e60b} ", markdown = "\u{e73e} ", vim = "\u{e62b} ",
		sh = "\u{f489} ", bash = "\u{f489} ", zsh = "\u{f489} ", rust = "\u{e7a8} ",
		go = "\u{e724} ", c = "\u{e61e} ", cpp = "\u{e61d} ", java = "\u{e738} ",
		php = "\u{e73d} ", ruby = "\u{e739} ", swift = "\u{e755} ", kotlin = "\u{e634} ",
		dart = "\u{e798} ", elixir = "\u{e62d} ", haskell = "\u{e777} ", sql = "\u{e706} ",
		yaml = "\u{f481} ", toml = "\u{e615} ", xml = "\u{f05c} ", dockerfile = "\u{f308} ",
		gitcommit = "\u{f418} ", gitconfig = "\u{f1d3} ", vue = "\u{fd42} ", svelte = "\u{e697} ", astro = "\u{e628} ",
	}
...
local function mode_icon()
	...
	local modes = {
		n = " \u{f121}  NORMAL", i = " \u{f11c}  INSERT", v = " \u{f016} VISUAL",
		V = " \u{f016} V-LINE", ["\22"] = " \u{f016} V-BLOCK", c = " \u{f120} COMMAND",
		s = " \u{f0c5} SELECT", S = " \u{f0c5} S-LINE", ["\19"] = " \u{f0c5} S-BLOCK",
		R = " \u{f044} REPLACE", r = " \u{f044} REPLACE", ["!"] = " \u{f489} SHELL", t = " \u{f120} TERMINAL",
	}
...
-- Consolidated & Optimized Format on Save
vim.api.nvim_create_autocmd("BufWritePre", {
	...
	callback = function(args)
		...
		pcall(vim.lsp.buf.format, {
			...
			async = true,
			filter = function(c)
				return has_efm and c.name == "efm" or true
			end,
		})
	end,
})
...
require("gitsigns").setup({
	signs = {
		add = { text = "\u{2590}" }, change = { text = "\u{2590}" }, delete = { text = "\u{2590}" },
		topdelete = { text = "\u{25e6}" }, changedelete = { text = "\u{25cf}" }, untracked = { text = "\u{25cb}" },
	},
	signcolumn = true,
...
-- NOTE: blink.cmp setup and capabilities are now handled safely at the top inside lazy.nvim
...
do
	...
	vim.lsp.config("efm", {
		filetypes = { "c", "cpp", "css", "go", "html", "javascript", "javascriptreact", "json", "jsonc", "lua", "markdown", "python", "sh", "typescript", "typescriptreact", "vue", "svelte", "tailwindcss" },
		init_options = { documentFormatting = true },
		settings = {
			languages = {
				c = { clangfmt, cpplint }, go = { gofumpt, go_revive }, cpp = { clangfmt, cpplint },
				css = { prettier_d }, html = { prettier_d }, javascript = { eslint_d, prettier_d },
				javascriptreact = { eslint_d, prettier_d }, json = { eslint_d, fixjson }, jsonc = { eslint_d, fixjson },
				lua = { luacheck, stylua }, markdown = { prettier_d }, python = { flake8, black },
				sh = { shellcheck, shfmt }, typescript = { eslint_d, prettier_d }, typescriptreact = { eslint_d, prettier_d },
				vue = { eslint_d, prettier_d }, svelte = { eslint_d, prettier_d },
			},
		},
	})
end

vim.lsp.enable({ "lua_ls", "pyright", "bashls", "ts_ls", "gopls", "clangd", "efm", "tailwindcss" })
...
-- Disable default <Tab> mapping so it doesn't conflict with blink.cmp
vim.g.copilot_no_tab_map = true

vim.keymap.set("i", "<M-l>", 'copilot#Accept("\\<CR>")', { expr = true, replace_keycodes = false, desc = "Copilot Accept Full" })
...
vim.keymap.set("i", "<M-e>", "<Plug>(copilot-dismiss)", { desc = "Copilot Dismiss Suggestion" })
